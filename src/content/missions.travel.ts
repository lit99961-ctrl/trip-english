...
function travelMission(
  ...
  return {
    ...
    productionPhrases: phrases.map(([suffix, english, chinese, intent, keywords, requiredKeywordGroups, recovery], index) => ({
      ...
      activeTarget: index < (["hotel-checkin", "directions-tickets", "restaurant", "urgent-help"].includes(id) ? 3 : 2)
    })),
    exercises: [
...
export const travelMissions: readonly Mission[] = [
  travelMission("hk-checkin", "香港机场值机", "Hong Kong", ["check-in", "passport", "baggage", "gate"], { bag: "one bag", seat: "window" }, "I have {bag}. Can I have a {seat} seat?", "CHECK-IN CLOSES BEFORE DEPARTURE", "bag", [
    ["check-in", "I would like to check in.", "我想办理值机。", "check-in", ["check in", "flight"], [["check in"]]],
    ["passport", "Here is my passport.", "这是我的护照。", "show-passport", ["passport", "document"], [["passport"]]],
    ...
    ["gate", "Which gate is it?", "是哪个登机口？", "find-gate", ["gate", "boarding"], [["gate"]]],
    ["repeat", "Could you say the gate again slowly?", "您能慢一点再说一遍登机口吗？", "recover-gate", ["again", "slowly"], [["again", "repeat"], ["slowly"]], true]
  ]),
  travelMission("helsinki-transfer", "赫尔辛基转机", "Helsinki", ["transfer", "security", "departure", "gate"], { gate: "32", time: "20 minutes" }, "Is gate {gate} this way? I have {time}.", "TRANSFER SECURITY → GATES 31–36", "gate", [
    ["transfer", "I am connecting to Rome.", "我要转机去罗马。", "state-connection", ["connecting", "Rome"], [["connecting", "connection"], ["Rome"]]],
    ["security", "Where is transfer security?", "转机安检在哪里？", "find-security", ["transfer", "security"], [["transfer"], ["security"]]],
    ["gate", "Is gate 32 this way?", "32号登机口是这边吗？", "confirm-gate", ["gate", "this way"], [["gate"]]],
  ...
  travelMission("rome-arrival", "罗马抵达", "Rome", ["arrivals", "baggage claim", "train", "ticket"], { train: "Leonardo Express", quantity: "two" }, "I need {quantity} tickets for the {train}.", "LEONARDO EXPRESS — ROME TERMINI — TICKETS", "express", [
    ["arrivals", "Where is the arrivals exit?", "到达出口在哪里？", "find-arrivals-exit", ["arrivals", "exit"], [["arrivals"], ["exit"]]],
    ["baggage", "Where is baggage claim?", "行李提取处在哪里？", "find-baggage", ["baggage", "claim"], [["baggage claim", "baggage"]]],
  ...
  travelMission("hotel-checkin", "酒店入住", "Rome and Lucerne", ["reservation", "self check-in", "key card", "breakfast", "Wi-Fi"], { keys: "two", room: "ready" }, "Is the room {room}? May I have {keys} key cards?", "SELF CHECK-IN: USE YOUR NAME AT THE KIOSK", "key", [
    ["reservation", "I have a reservation.", "我有预订。", "check-reservation", ["reservation", "booking"], [["reservation", "booking"]]],
    ["name", "The name is Li.", "名字是李。", "give-name", ["name", "Li"], [["Li"]]],
  ...
  travelMission("directions-tickets", "问路和买票", "Rome and Vatican", ["entrance", "ticket", "metro", "Vatican Museums", "opening time"], { entrance: "B", ticket: "day ticket" }, "Where is entrance {entrance}? I need a {ticket}.", "VATICAN MUSEUMS — OPENING TIMES — LAST ENTRY", "entrance", [
    ["direction", "How do I get to the metro?", "我怎么去地铁？", "ask-direction", ["how", "metro"], [["metro"]]],
    ["entrance", "Where is entrance B?", "B入口在哪里？", "find-entrance", ["entrance", "B"], [["entrance"], ["B"]]],
  ...
  travelMission("restaurant", "餐厅点餐", "Florence", ["table", "menu", "water", "allergy", "bill"], { table: "two people", item: "pasta" }, "A table for {table}. I would like {item}.", "MENU NOTICE: PLEASE TELL US ABOUT ALLERGIES", "table", [
    ["table", "A table for two, please.", "请给两个人一张桌子。", "request-table", ["table", "two"], [["table"], ["two", "2"]]],
    ["menu", "May I see the menu?", "我可以看菜单吗？", "request-menu", ["menu", "see"], [["menu"]]],
    ["order", "I would like this pasta.", "我想要这份意面。", "order-food", ["pasta", "this"], [["pasta"]]],
  ...
  travelMission("italy-high-speed-rail", "意大利高铁", "Rome–Florence–Venice", ["platform", "carriage", "seat", "delay", "departure"], { platform: "9", carriage: "4" }, "Is this train on platform {platform}? Where is carriage {carriage}?", "DEPARTURES: FLORENCE / VENICE — PLATFORM 9", "platform", [
    ["train", "Is this the train to Florence?", "这是去佛罗伦萨的火车吗？", "confirm-train", ["train", "Florence"], [["train"], ["Florence"]]],
    ["platform", "Which platform is it?", "是哪个站台？", "find-platform", ["platform", "track"], [["platform", "track"]]],
  ...
  travelMission("venice-vaporetto", "威尼斯水上巴士", "Venice", ["vaporetto", "stop", "line", "validate", "get off"], { line: "1", stop: "Rialto" }, "Does line {line} stop at {stop}?", "LINE 1 — RIALTO — VALIDATE TICKET", "line", [
  ...
  travelMission("milan-swiss-transfer", "米兰转乘瑞士火车", "Milan–Interlaken", ["change", "platform", "border", "delay", "Interlaken"], { destination: "Interlaken", platform: "6" }, "Is this for {destination}? Which platform is {platform}?", "INTERLAKEN — CHANGE TRAINS — PLATFORM 6", "platform", [
    ["interlaken", "Is this train for Interlaken?", "这趟车去因特拉肯吗？", "confirm-destination", ["train", "Interlaken"], [["Interlaken"]]],
    ["change", "Where do I change trains?", "我在哪里换车？", "ask-change", ["change", "trains"], [["change trains", "change"]]],
  ...
  travelMission("swiss-mountain-transit", "瑞士山区交通", "Zermatt–Gornergrat–Grindelwald–Lucerne", ["regional train", "cable car", "Grindelwald", "Interlaken", "Lungern", "Lucerne", "Zurich Airport", "weather", "last train"], { destination: "Lucerne", returnTime: "later" }, "Is this for {destination}? Is there a {returnTime} train back?", "TIMETABLE: LUNGERN STOP — LUCERNE — ZURICH AIRPORT", "train", [
    ["train", "Which train goes to Zermatt?", "哪趟火车去采尔马特？", "find-regional-train", ["train", "Zermatt"], [["train"]]],
    ["cable", "Where is the cable car?", "缆车在哪里？", "find-cable-car", ["cable car", "lift"], [["cable car", "lift"]]],
    ...
    ["return", "What is the last train back?", "最后一趟回程火车是几点？", "ask-last-return", ["last", "back"], [["last"], ["train", "return"]]],
  ...
  travelMission("shopping-tax-refund", "购物和退税", "Milan", ["size", "color", "card", "tax-free", "refund"], { size: "medium", color: "blue" }, "Do you have this in {size} and {color}?", "TAX-FREE: PASSPORT REQUIRED FOR REFUND FORM", "size", [
    ["size", "Do you have this in medium?", "这个有中码吗？", "ask-size", ["medium", "size"], [["medium"]]],
    ["color", "Do you have it in blue?", "有蓝色的吗？", "ask-color", ["blue", "color"], [["blue"]]],
  ...
  travelMission("urgent-help", "紧急求助", "Italy and Switzerland", ["lost", "toilet", "police", "doctor", "pharmacy", "112", "location", "bag"], { place: "station entrance", help: "a police officer" }, "I am lost at {place}. I need {help}.", "WC / TOILET → EMERGENCY 112", "lost-separated", [
    ["lost-separated", "I am lost and separated from my group.", "我迷路了，和同伴走散了。", "report-lost-and-separated", ["lost", "separated"], [["lost"]]],
